...
1. **Non-atomic metadata save can destroy the vault index. — FIXED**
...
2. **Corrupted/unreadable metadata is silently replaced by an empty list, then persisted.**
...
9. **`toggleSign` reformats typed input and loses trailing decimal digits.**
...
19. **Unbounded growth: calculator history and PIN-less input length.**
